...
/// <summary>
/// Tests for the <see cref="SDEventStream"/> class.
/// </summary>
public class SDEventStreamTest
{
    ...
    public void TestEventOutcomes()
    {
        ...
        IObjectStream<SentenceSample?> sampleStream =
            ObjectStreamUtils.CreateObjectStream(sample);

        Factory factory = new Factory();

        IObjectStream<Event?> eventStream = new SDEventStream(sampleStream,
            factory.CreateSentenceContextGenerator("eng"),
            factory.CreateEndOfSentenceScanner("eng"));

        ClassicAssert.AreEqual(SentenceDetectorME.NO_SPLIT, eventStream.Read()?.Outcome);
        ClassicAssert.AreEqual(SentenceDetectorME.SPLIT, eventStream.Read()?.Outcome);
        ClassicAssert.AreEqual(SentenceDetectorME.NO_SPLIT, eventStream.Read()?.Outcome);
        ClassicAssert.AreEqual(SentenceDetectorME.SPLIT, eventStream.Read()?.Outcome);
