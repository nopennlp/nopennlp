...
/// <summary>
/// Tests for the <see cref="TokenizerModel"/> class.
/// </summary>
public class TokenizerModelTest
{
    [Test]
    public void TestSentenceModel()
    {
        TokenizerModel model = TokenizerTestUtil.CreateSimpleMaxentTokenModel();

        MemoryStream arrayOut = new MemoryStream();
        model.Serialize(arrayOut);
        arrayOut.Dispose();

        model = new TokenizerModel(new MemoryStream(arrayOut.ToArray()));
        // TODO: check that both maxent models are equal

        // Also test serialization after building model from an inputstream
        arrayOut = new MemoryStream();
        model.Serialize(arrayOut);
        arrayOut.Dispose();

        new TokenizerModel(new MemoryStream(arrayOut.ToArray()));
