...
public class TwoPassDataIndexerTest
{
    ...
    // NOpenNLP: upstream also has testIndexWithNewline, which builds its events
    // with NameFinderEventStream over a NameSample. Neither is ported yet -- they
    // belong to the name finder's training path -- so that test is omitted rather
    // than stubbed. It should be added when NameSample and NameFinderEventStream
    // arrive.
}
