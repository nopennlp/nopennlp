...
public class ConlluPOSSampleStreamTest
{
    ...
    public void TestParseContraction()
    {
        ...
        using var stream = new ConlluPOSSampleStream(new ConlluStream(streamFactory), ConlluTagset.U);

        var expected = POSSample.Parse("Numa_ADP+DET reunião_NOUN entre_ADP " +
        ...
        ClassicAssert.AreEqual(expected, predicted);
    }
