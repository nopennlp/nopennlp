...
public class IrishSentenceBankDocumentTest
{
    ...
    {
        using var irishSBXmlIn = TestResources.OpenResource("/opennlp/tools/formats/irishsentencebank/irishsentencebank-sample.xml");
        var doc = IrishSentenceBankDocument.Parse(irishSBXmlIn);
        ...
        CollectionAssert.AreEqual(new[] { "a" }, flex[0].Flex);

        var flex2 = sent2.Flex;
        ClassicAssert.AreEqual("ón", flex2![4].Surface);
        CollectionAssert.AreEqual(new[] { "ó", "an" }, flex2[4].Flex);

        ClassicAssert.AreEqual("Excuse me, are you from the stone age?", sent2.Translation);

        var ts = sent1.TokenSample;
        var spans = ts.TokenSpans;
