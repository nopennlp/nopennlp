...
public sealed class LemmatizerEvaluatorTool : AbstractEvaluatorTool<LemmaSample?>
{
    ...
    {
        var model = new LemmatizerModelLoader().Load(parseResult.GetValueByName(this.model)!);
        ...
        if (reportFile != null)
        {
            ...
            {
                throw new TerminateToolException(-1, $"IO error while creating Lemmatizer fine-grained report file: {e.Message}");
            }
        }
