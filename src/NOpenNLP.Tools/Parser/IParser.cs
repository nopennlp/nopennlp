...
public interface IParser
{
    ...
    /// <para/>
    /// <b>Note:</b> The nodes within the returned parses are shared with other parses and
    /// therefore their parent node references will not be consistent with their child node
    /// reference. <see cref="Parse.Parent"/> can be used to make the parents consistent with
    /// a particular parse, but subsequent calls to <see cref="AbstractBottomUpParser.SetParents"/>
    ...
    /// <returns>
    /// A full parse of the specified tokens or the flat chunks of the tokens if a
