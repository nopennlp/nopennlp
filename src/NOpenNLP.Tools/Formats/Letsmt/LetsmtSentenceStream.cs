...
internal class LetsmtSentenceStream : ObjectStreamBase<SentenceSample?>
{
    private readonly LetsmtDocument source;
    ...
    private IEnumerator<LetsmtDocument.LetsmtSentence> sentenceIt;

    internal LetsmtSentenceStream(LetsmtDocument source)
    {
        this.source = source;
    ...
    public override SentenceSample? Read()
    {
        ...
        for (int i = 0; i < 25 && sentenceIt.MoveNext(); i++)
        {
            ...
            {
                sentencesString.Append(string.Join(" ", sentence.Tokens));
            }
            else if (sentence.NonTokenizedText != null)
            {
                sentencesString.Append(sentence.NonTokenizedText);
            }
