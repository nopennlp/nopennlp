...
public class Conll02NameSampleStreamFactory : LanguageSampleStreamFactory<NameSample?>
{
    ...
    private static readonly IFormatParameter LangParam =
        new FormatParameter<string>("-lang", "spa|nld");
    ...
    public override IObjectStream<NameSample?> Create(IFormatParameterValues values)
    {
        ...
        if ("nl".Equals(langValue, StringComparison.Ordinal)
            || "nld".Equals(langValue, StringComparison.Ordinal))
        {
            lang = Conll02NameSampleStream.Language.NLD;
            language = langValue;
        }
        else if ("es".Equals(langValue, StringComparison.Ordinal)
            || "spa".Equals(langValue, StringComparison.Ordinal))
        {
