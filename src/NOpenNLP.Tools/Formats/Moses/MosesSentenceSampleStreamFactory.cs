...
/// <summary>
/// Factory producing OpenNLP <see cref="MosesSentenceSampleStream"/>s.
/// </summary>
public class MosesSentenceSampleStreamFactory : AbstractSampleStreamFactory<SentenceSample?>
{
    ...
    public override IObjectStream<SentenceSample?> Create(IFormatParameterValues values)
    {
        ...
        try
        {
            // NOpenNLP: ReadData does upstream's checkInputFile("Data", ...),
            // createInputStreamFactory and PlainTextByLineStream in one call.
            lineStream = ReadData(values);
        }
