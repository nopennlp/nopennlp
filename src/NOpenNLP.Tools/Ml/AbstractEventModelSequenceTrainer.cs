...
/// <summary>
/// Base class for trainers which produce a <see cref="IMaxentModel"/> from a stream of sequences.
/// </summary>
...
public abstract class AbstractEventModelSequenceTrainer<T> : AbstractTrainer, IEventModelSequenceTrainer<T>
{
    ...
    /// </summary>
    public abstract IMaxentModel DoTrain(ISequenceStream<T> events);

    public IMaxentModel Train(ISequenceStream<T> events)
    {
        Validate();

        IMaxentModel model = DoTrain(events);
        AddToReport(TRAINER_TYPE_PARAM, EventModelSequenceTrainer.SEQUENCE_VALUE);
        return model;
    }
}
