...
public class BinaryPerceptronModelWriter : PerceptronModelWriter
{
    ...
    /// <summary>
    /// Constructor which takes a <see cref="PerceptronModel"/> and a <see cref="FileInfo"/> and
    ...
    /// </summary>
    /// <param name="model">The <see cref="PerceptronModel"/> which is to be persisted.</param>
    /// <param name="f">The file in which the model is to be persisted.</param>
    public BinaryPerceptronModelWriter(AbstractModel model, FileInfo f)
        : base(model)
    {
    ...
    /// <summary>
    /// Constructor which takes a <see cref="PerceptronModel"/> and a <see cref="Stream"/> and
    /// prepares itself to write the model to that stream.
    /// </summary>
    /// <param name="model">The <see cref="PerceptronModel"/> which is to be persisted.</param>
    /// <param name="dos">The stream which will be used to persist the model.</param>
    public BinaryPerceptronModelWriter(AbstractModel model, Stream dos)
        : base(model)
    {
        output = dos;
    }
