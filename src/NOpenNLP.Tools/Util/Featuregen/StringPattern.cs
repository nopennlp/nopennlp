...
public class StringPattern
{
    ...
    public static StringPattern Recognize(string token)
    {
        ...
        for (int i = 0; i < token.Length; i++)
        {
            ...
            var us = UnicodeScriptResolver.Of(codePoint);
            if (us != UnicodeScript.Common)
            {
                if (us == UnicodeScript.Latin)
                {
                    pattern &= ~(ALL_HIRAGANA | ALL_KATAKANA);
                }
                else if (us == UnicodeScript.Han)
                {
                    pattern &= ~(ALL_HIRAGANA | ALL_KATAKANA | ALL_LOWERCASE_LETTER);
                }
                else if (us == UnicodeScript.Hiragana)
                {
                    pattern &= ~(ALL_KATAKANA | ALL_LOWERCASE_LETTER);
                }
                else if (us == UnicodeScript.Katakana)
                {
